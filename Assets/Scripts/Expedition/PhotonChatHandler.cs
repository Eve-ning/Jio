...
namespace DIPProject
{
    public class PhotonChatHandler : MonoBehaviour, IChatClientListener
    {
        ...
        [Tooltip("The InputField for sending a message")] [SerializeField]
        private InputField chatInput;

        [Tooltip("The TextWindow")] [SerializeField]
        private Text chatWindow;

        [Tooltip("The Button to send a message")] [SerializeField]
        private Button chatBtn;

        [Tooltip(
            "The Fishing Handler. This is to detect if it's currently fishing. If it is, then we disable chat boxes.")]
        [SerializeField]
        private ExpeditionFishingHandler expeditionFishingHandler;
        ...
        {
            chatClient.Subscribe(new[] {PhotonNetwork.CurrentRoom.Name});
            ToggleChat(true);
        }

        public void OnDisconnected()
        {
            chatClient.Unsubscribe(new[] {PhotonNetwork.CurrentRoom.Name});
            ToggleChat(false);
        }
        ...
        private void MsgRoom(string msg)
        {
            var result = chatClient.PublishMessage(PhotonNetwork.CurrentRoom.Name, msg);
        ...
        {
            for (var i = 0; i < senders.Length; i++) chatWindow.text += "\n" + senders[i] + ": " + messages[i];
        }
